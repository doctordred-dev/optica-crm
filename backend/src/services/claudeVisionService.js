          ...
          prescription: {
            ...
            properties: {
              ...
              pd: {
                type: 'string',
                description:
                  'Міжзрачкова відстань, в мм. Одне значення — просто число рядком (напр. "63"). Якщо на бланку явно вказано ДІАПАЗОН/два різних значення (напр. через дріб "34,5/32" або тире "66-68") — запиши як діапазон через ОДНЕ тире, менше значення першим, десяткові через крапку (напр. "32-34.5"). "" якщо не видно.'
              },
              purposeRaw: {
                type: 'string',
...
- DP / Dp = міжзрачкова відстань (pd), в мм. Якщо на бланку одне значення — просто число. Якщо вказано ДВА різних значення (напр. через дріб "34,5/32", або окремо для дали/близу, або вже через тире) — це діапазон, запиши його рядком через ОДНЕ тире, менше число першим (напр. "32-34.5"), десяткові через крапку, НЕ кому. Якщо є окремий повніший бланк рецепта — бери значення з нього; якщо тільки один бланк — вкажи як є і додай примітку за потреби.
- П.І.П. = ПІБ клієнта
